  ...
  (dispatch: Dispatch) => {
    const fetchData = async () => {
      try {
        ...
        const mappedQuestions = fetchedData.map((q: Question) => {
          return {
            ...q,
      ...
      try {
        ...
        Object.keys(translationData).map((germanTerm: string) => {
          if (!germanTerm.includes("languagePair")) {
            const translationObj = translationData[germanTerm as keyof typeof translationData]
            remappedTranslations[germanTerm.toLowerCase()] = translationObj.en
            return germanTerm;
          }
          return;
        })
        dispatch(getTranslationsSuccess({
